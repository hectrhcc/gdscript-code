...
func _ready():
	hide_textbox()
	add_text("NO IMPORTA DONDE ESTES...  LA MAGIA COMIENZA CONTIGO","INICIAR","INICIAR")
...
func add_text(text,animal,memoria):
	...
	label3.text = memoria
	show_textbox()
	$CanvasLayer/Tween.interpolate_property(label,"percent_visible",1.0, 1.0, len(text) * CHAR_READ_RATE, Tween.TRANS_LINEAR, Tween.EASE_IN_OUT)
...
func seleccion(enter):
	if(opc_act==0):
		CambioEscena.cambio_escena("res://scenes/002a.tscn")	
	if(opc_act==1):
		CambioEscena.cambio_escena("res://scenes/002a.tscn")
